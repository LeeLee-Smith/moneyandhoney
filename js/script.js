...
document.querySelector('.current-date').textContent = `${now.toLocaleDateString('pl', {month: "long", day: "2-digit", year: "numeric",  hour: "2-digit", minute: "2-digit"})}`;
...
document.querySelector('.data-end-scr').textContent = `${now.toLocaleDateString('pl', {month: "long", day: "2-digit", year: "numeric"})}`;
...
inputValue.addEventListener('input', function(e) {
    ...
    profit.textContent = `${this.value}` * 3960;

    if(this.value == 0) {
        profit.textContent = 3960;
        days.textContent = 1;
    }
})